...
void ElasticitySystem::init_data() {
    ...
    std::set<boundary_id_type> all_boundary_ids, dirichlet_boundary_ids, dirichlet_u_boundary_ids, dirichlet_v_boundary_ids;
    all_boundary_ids = this->get_mesh().get_boundary_info().get_boundary_ids();
    this->comm().set_union(all_boundary_ids);

    if (all_boundary_ids.count(boundary_id_min_x)) dirichlet_boundary_ids.insert(boundary_id_min_x);
    if (all_boundary_ids.count(node_boundary_id)) dirichlet_boundary_ids.insert(node_boundary_id);
    if (all_boundary_ids.count(edge_boundary_id)) dirichlet_boundary_ids.insert(edge_boundary_id);
    if (all_boundary_ids.count(fixed_u_boundary_id)) dirichlet_u_boundary_ids.insert(fixed_u_boundary_id);
    if (all_boundary_ids.count(fixed_v_boundary_id)) dirichlet_v_boundary_ids.insert(fixed_v_boundary_id);

    std::vector<unsigned int> variables{_u_var};
    if (_dim > 1) variables.push_back(_v_var);
    if (_dim > 2) variables.push_back(_w_var);

    ZeroFunction<> zf;

    // Most DirichletBoundary users will want to supply a "locally
    // indexed" functor
    DirichletBoundary dirichlet_bc(dirichlet_boundary_ids, variables, zf, LOCAL_VARIABLE_ORDER);
    this->get_dof_map().add_dirichlet_boundary(dirichlet_bc);

    if (!dirichlet_u_boundary_ids.empty()) {
        DirichletBoundary dirichlet_u_bc(dirichlet_u_boundary_ids, {_u_var}, zf, LOCAL_VARIABLE_ORDER);
        this->get_dof_map().add_dirichlet_boundary(dirichlet_u_bc);
    }

    if (!dirichlet_v_boundary_ids.empty()) {
        DirichletBoundary dirichlet_v_bc(dirichlet_v_boundary_ids, {_v_var}, zf, LOCAL_VARIABLE_ORDER);
...
bool ElasticitySystem::element_time_derivative(bool request_jacobian, DiffContext& context) {
    FEMContext& c = cast_ref<FEMContext&>(context);
    ...
    std::reference_wrapper<DenseSubVector<Number>> F[3] = {c.get_elem_residual(u_dot_var), c.get_elem_residual(v_dot_var), c.get_elem_residual(w_dot_var)};
    ...
    for (unsigned int qp = 0; qp != n_qpoints; qp++) {
        ...
        if (_dim > 1) c.interior_gradient(_v_var, qp, grad_v);
        if (_dim > 2) c.interior_gradient(_w_var, qp, grad_w);
        ...
        for (unsigned int i = 0; i < _dim; i++)
            for (unsigned int j = 0; j < _dim; j++)
                for (unsigned int k = 0; k < _dim; k++)
                    for (unsigned int l = 0; l < _dim; l++) tau(i, j) += elasticity_tensor(i, j, k, l) * grad_U(k, l);
                ...
                if (request_jacobian) {
                    for (unsigned int j = 0; j != n_u_dofs; j++) {
                        for (unsigned int beta = 0; beta < _dim; beta++) {
                            ...
                            for (unsigned int k = 0; k < _dim; ++k)
                                for (unsigned int l = 0; l < _dim; ++l) K[k][l](i, j) += elasticity_tensor(k, alpha, l, beta) * c0 * grad_phi[i][qp](alpha) * JxW[qp];
                        }
                    }
                }
            }
...
bool ElasticitySystem::side_time_derivative(bool request_jacobian, DiffContext& context) {
    ...
    if (c.has_side_boundary_id(traction_boundary_id) || c.has_side_boundary_id(pressure_boundary_id)) {
        // If we have an unsteady solver, then we need to extract the corresponding
        // velocity variable. This allows us to use either a FirstOrderUnsteadySolver
        // or a SecondOrderUnsteadySolver. That is, we get back the velocity variable
        // index for FirstOrderUnsteadySolvers or, if it's a SecondOrderUnsteadySolver,
        ...
        const bool pressureforce = c.has_side_boundary_id(pressure_boundary_id);

        for (unsigned int qp = 0; qp != n_qpoints; qp++) {
            if (pressureforce) traction = pressure * normals[qp];

            for (unsigned int i = 0; i != n_u_dofs; i++)
                for (unsigned int d = 0; d < _dim; ++d) F[d](i) -= traction(d) * phi[i][qp] * JxW[qp];
        }
    }
...
bool ElasticitySystem::mass_residual(bool request_jacobian, DiffContext& context) {
    ...
    std::reference_wrapper<DenseSubMatrix<Number>> Kdiag[3] = {c.get_elem_jacobian(u_dot_var, u_dot_var), c.get_elem_jacobian(v_dot_var, v_dot_var), c.get_elem_jacobian(w_dot_var, w_dot_var)};
    ...
    for (unsigned int qp = 0; qp != n_qpoints; qp++) {
        ...
        if (_dim > 1) c.interior_accel(v_dot_var, qp, accel[1]);
        if (_dim > 2) c.interior_accel(w_dot_var, qp, accel[2]);

        for (unsigned int i = 0; i != n_u_dofs; i++) {
            for (unsigned int d = 0; d < _dim; ++d) F[d](i) += _rho * accel[d] * phi[i][qp] * JxW[qp];

            if (request_jacobian) {
                for (unsigned int j = 0; j != n_u_dofs; j++) {
                    Real jac_term = context.get_elem_solution_accel_derivative() * _rho * phi[i][qp] * phi[j][qp] * JxW[qp];

                    for (unsigned int d = 0; d < _dim; ++d) Kdiag[d](i, j) += jac_term;
                }
            }
        }
    }
...
Real ElasticitySystem::elasticity_tensor(unsigned int i, unsigned int j, unsigned int k, unsigned int l) {
    ...
    return lambda_1 * kronecker_delta(i, j) * kronecker_delta(k, l) + lambda_2 * (kronecker_delta(i, k) * kronecker_delta(j, l) + kronecker_delta(i, l) * kronecker_delta(j, k));
}